...
void blob_xmit(unsigned char group)
{

	// group = which group of 15 board to transmit, with each board receiving 2 bytes (4 bits each of RGBA)
	// xmit packet is
		//group number
		//flags
		//15 balloons of data, 2 bytes each
	unsigned char loop,i=0, int_data[35], outgoing[35], start_offset=0;

	LED1_IO^=1;
	start_offset=(group*30)+2;

	outgoing[0]=group;
	outgoing[1]=xmit_frame[1];
	for(i=0;i<30;i++)
		outgoing[i+2]=xmit_frame[i+start_offset];
...
//	nrf24l01_initialize_debug(false, 32, false); //initialize the 24L01 to the debug configuration as TX, 1 data byte, and auto-ack disabled
    ...
    {     
        ...
        if(TickGet() - t >= TICK_SECOND/50ul)
        {
            t = TickGet();
		//	test_xmit();
		//	nrf24l01_get_all_registers((unsigned char*)&reg_ptr);
			blob_xmit(group_counter);  //new style blob code on this metric here, 50Hz
			group_counter++;
			if(group_counter>6)
				group_counter=0;
        }

		//this is for the older transmit style
		if(new_balloon_pack)
		{
			new_balloon_pack=0;
			nordic_XMIT();
